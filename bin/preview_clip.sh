...
rest=$(printf '%s' "${text}" | cut -c2-)
text="$(printf '%s' "${first}" | tr '[:lower:]' '[:upper:]')${rest}"

## Trunca
if [ "${#text}" -gt "${MAX_QUOTE_LEN}" ]; then
    text=$(printf '%s' "${text}" | cut -c1-"${MAX_QUOTE_LEN}")
    text="${text}..."
fi

## Exibe preview na tela via fbink
if [ "${FBINK_BIN}" != "true" ]; then
    short=$(printf '%s' "${text}" | cut -c1-55)
...
label_pub=$(je "Publicar: \"$(printf '%s' "${text}" | cut -c1-38)...\"")
